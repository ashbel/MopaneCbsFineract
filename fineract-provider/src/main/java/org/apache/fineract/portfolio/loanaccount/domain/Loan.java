...
public class Loan extends AbstractPersistableCustom<Long> {
    ...
    //Ashbel 01 April 2020 Get Capitalised Charges
    private BigDecimal deriveSumTotalOfChargesCapitalisedDisbursement() {
            ...
            for (LoanTransaction loanTransaction : accruals) {
                if (loanTransaction.getTransactionDate().isAfter(installment.getFromDate())
                  ...
                  for(LoanCharge loanCharge : loan.charges) {
                    ...
                    logger.info(installment.getInstallmentNumber() +" T_Int " + interest + " I_Int " + installment.getInterestCharged(getCurrency()) + " T_Fee " + fee + " I_Fee " + installment.getFeeChargesCharged(getCurrency()));
    ...
    public ChangedTransactionDetail disburse(final AppUser currentUser, final JsonCommand command, final Map<String, Object> actualChanges,
        ...
        updateLoanSummaryDerivedFields();
        final Money interestApplied = Money.of(getCurrency(), this.summary.getTotalInterestCharged());
//        final Money upfrontCharges = Money.of(getCurrency(), deriveSumTotalOfChargesCapitalisedDisbursement());
         ...
         * selected
         **/
//        if(upfrontCharges.isGreaterThanZero()) {
//            final LoanTransaction chargesAppliedTransaction = LoanTransaction.accrueLoanCharge(this,getOffice(),  upfrontCharges,
//                    actualDisbursementDate,Money.zero(getCurrency()),Money.zero(getCurrency()), createdDate, currentUser);
//            addLoanTransaction(chargesAppliedTransaction);
//            updateLoanOutstandingBalaces();
//        }
    ...
    private void handleDisbursementTransaction(final LocalDate disbursedOn, final LocalDateTime createdDate, final AppUser currentUser,
        ...
        Money accruedMoney = Money.zero(getCurrency());
        ...
        final LoanTransaction accrueCharge =  LoanTransaction.accrueAtDisbursement(getOffice(), disbursentMoney, paymentDetail,
                disbursedOn, null, createdDate, currentUser);

        final Integer installmentNumber = null;
        for (final LoanCharge charge : charges()) {
            Date actualDisbursementDate = getActualDisbursementDate(charge);
            if (((charge.getCharge().getChargeTimeType() == ChargeTimeType.DISBURSEMENT.getValue()) //|| charge.getCharge().getChargeTimeType() == ChargeTimeType.DISBURSEMENT_CAPITALISED.getValue())
...
//            if (charge.getCharge().getChargeTimeType() == ChargeTimeType.DISBURSEMENT_CAPITALISED.getValue()) {
//                accruedMoney = accruedMoney.plus(charge.amount());                
//            }
        }
...
//        if (accruedMoney.isGreaterThanZero()) {
//            final Money zero = Money.zero(getCurrency());
//            accrueCharge.updateComponentsAndTotal(zero, zero, accruedMoney, zero);
//            accrueCharge.updateLoan(this);
//            addLoanTransaction(accrueCharge);
//            updateLoanOutstandingBalaces();
//        }
    ...
    private ChangedTransactionDetail handleRepaymentOrRecoveryOrWaiverTransaction(final LoanTransaction loanTransaction,
      ...
      if (isTopup()) {
        ...
        if (reprocess) {
            logger.info("Reprocess True");
            if (this.repaymentScheduleDetail().isInterestRecalculationEnabled()) {
            	logger.info("InterestRecalculationEnabled");
                regenerateRepaymentScheduleWithInterestRecalculation(scheduleGeneratorDTO, currentUser);
            }
            final List<LoanTransaction> allNonContraTransactionsPostDisbursement = retreiveListOfTransactionsPostDisbursement();
            changedTransactionDetail = loanRepaymentScheduleTransactionProcessor.handleTransaction(getDisbursementDate(),
    ...
    //i want to get transactions with accruals to update transaction balances in m_loan_transactions tables
    private List<LoanTransaction> retreiveListOfTransactionsWithAccruals() {
        final List<LoanTransaction> repaymentsOrWaiversOrAccruals = new ArrayList<>();
        for (final LoanTransaction transaction : this.loanTransactions) {
            if (transaction.isNotReversed() &&  !transaction.isNonMonetaryTransaction()) {
                repaymentsOrWaiversOrAccruals.add(transaction);
            }
        }
        final LoanTransactionComparator transactionComparator = new LoanTransactionComparator();
        Collections.sort(repaymentsOrWaiversOrAccruals, transactionComparator);
        return repaymentsOrWaiversOrAccruals;
    }
    ...
    private void updateLoanOutstandingBalaces() {
        ...
        List<LoanTransaction> loanTransactions = retreiveListOfTransactionsWithAccruals();
...
//        	loanTransaction.isIncomePosting() ||
            if (loanTransaction.isDisbursement() || loanTransaction.isIncomePosting() || loanTransaction.isFee() || loanTransaction.isAccrual() 
                ...
                if (this.loanInterestRecalculationDetails != null
                        ...
                        && !loanTransaction.isRepaymentAtDisbursement() && !loanTransaction.isIncomePosting()) {
    ...
    public LoanRepaymentScheduleInstallment fetchLoanForeclosureDetail(final LocalDate closureDate) {        
        ...
        Money totalPrincipal = (Money.of(getCurrency(), this.getSummary().getTotalPrincipalOutstanding().add(this.getSummary().getTotalFeeChargesOutstanding())));
        totalPrincipal = totalPrincipal.minus(receivables[3]).minus(receivables[4]);
        final Set<LoanInterestRecalcualtionAdditionalDetails> compoundingDetails = null;
        final LocalDate currentDate = DateUtils.getLocalDateOfTenant();
    ...
    public Money[] retriveIncomeOutstandingTillDate(final LocalDate paymentDate) {
        ...
        Money capitalisedCharge = Money.zero(currency);
        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
            if (!installment.getDueDate().isAfter(paymentDate)) {
                ...
                capitalisedCharge = balancesForCurrentPeroid[6];
        ...
        balances[4] = capitalisedCharge;
        return balances;
    }
    
    //Added this method to calculate accrued interest before applying it Ashbel 20/03/2020
    public Money[] retriveAccruedInterestTillToday(final LocalDate paymentDate) {
        Money[] balances = new Money[5];         
        final MonetaryCurrency currency = getCurrency();      
        Money interest = Money.zero(currency);
        Money paidFromFutureInstallments = Money.zero(currency); 
        ...
        Money penalty = Money.zero(currency);         
        for (final LoanRepaymentScheduleInstallment installment : this.repaymentScheduleInstallments) {
        	//if fromDate id less than todays date and if duedate is greater than todays date
            if (installment.getFromDate().isBefore(paymentDate) && installment.getDueDate().isAfter(paymentDate)) {
                Money[] balancesForCurrentPeroid = fetchInterestFeeAndPenaltyTillDate(paymentDate, currency, installment);                        
                if (balancesForCurrentPeroid[0].isGreaterThan(balancesForCurrentPeroid[5])) {
                    interest = interest.plus(balancesForCurrentPeroid[0])
                            .minus(balancesForCurrentPeroid[5]);
    ...
    private Money[] fetchInterestFeeAndPenaltyTillDate(final LocalDate paymentDate, final MonetaryCurrency currency, final LoanRepaymentScheduleInstallment installment) {
        ...
        Money capitalisedFees = Money.zero(getCurrency());
        Money capitalisedFeesAccountedFor = Money.zero(getCurrency());
...
//					feeForCurrentPeriod = feeForCurrentPeriod.plus(loanCharge.getAmount(currency));
					capitalisedFees = capitalisedFees.plus(loanCharge.getAmount(currency));
//					feeAccountedForCurrentPeriod = feeAccountedForCurrentPeriod
//							.plus(loanCharge.getAmountWaived(getCurrency()).plus(
//									loanCharge.getAmountPaid(getCurrency())));
					capitalisedFeesAccountedFor = capitalisedFeesAccountedFor
							.plus(loanCharge.getAmountWaived(getCurrency()).plus(
									loanCharge.getAmountPaid(getCurrency())));
//					feeForCurrentPeriod = feeForCurrentPeriod.minus(feeAccountedForCurrentPeriod);
				}
			}
		}
        ...
        balances[1] = feeForCurrentPeriod.plus(capitalisedFees).minus(capitalisedFeesAccountedFor);
        ...
        balances[6] = capitalisedFees.minus(capitalisedFeesAccountedFor);
        return balances;
    }
    ...
    public void updateInstallmentsPostDate(LocalDate transactionDate) {
        ...
        logger.info("UpdateInstallmentFees " + balances[1].getAmount());
        logger.info("UpdateInterest " + balances[0].getAmount());
        ...
        for (LoanCharge loanCharge : charges) {
            if (loanCharge.getDueLocalDate() != null
                ...
                loanCharge.setActive(false);
                logger.info("Loan Charge Set to False ");
