...
import org.apache.fineract.mopane.ssb.service.SsbPayImportWritePlatformService;
...
@Component
@Scope("singleton")
public class SsbPayImportApiResource {

    private static final Set<String> BATCH_RESPONSE_PARAMS = new HashSet<>(Arrays.asList("id", "bureau", "filename", "uploadedBy",
            "uploadedOn", "dryRun", "paymentTypeId", "postedCount", "failedCount", "needsReviewCount", "totalCount", "rows"));

    private static final Set<String> ROW_RESPONSE_PARAMS = new HashSet<>(Arrays.asList("id", "batchId", "rowNumber", "recId",
            "deductionCode", "reference", "idNumber", "ecNumber", "transDate", "amount", "name", "status", "reason", "suggestedLoanId",
            "suggestedAccountNo", "postedLoanId", "postedTransactionId", "note"));

    private final PlatformSecurityContext context;
    private final SsbPayImportWritePlatformService writePlatformService;
    private final DefaultToApiJsonSerializer<SsbPayBatchData> batchSerializer;
    private final DefaultToApiJsonSerializer<SsbPayRowData> rowSerializer;
    private final ApiRequestParameterHelper apiRequestParameterHelper;

    @Autowired
    public SsbPayImportApiResource(final PlatformSecurityContext context, final SsbPayImportWritePlatformService writePlatformService,
            final DefaultToApiJsonSerializer<SsbPayBatchData> batchSerializer,
            final DefaultToApiJsonSerializer<SsbPayRowData> rowSerializer, final ApiRequestParameterHelper apiRequestParameterHelper) {
    ...
    public Response upload(@FormDataParam("file") final InputStream uploadedInputStream,
        ...
        user.validateHasPermissionTo("CREATE_" + SsbConstants.RESOURCE_PAY_IMPORT);

        final String filename = fileDetail == null ? null : fileDetail.getFileName();
        return this.writePlatformService.processUpload(uploadedInputStream, filename, bureau, paymentTypeId, Boolean.TRUE.equals(dryRun),
                user);
    }
    ...
    public String batches(@Context final UriInfo uriInfo, @QueryParam("offset") final Integer offset,
        ...
        this.context.authenticatedUser().validateHasPermissionTo("CREATE_" + SsbConstants.RESOURCE_PAY_IMPORT);
        final Collection<SsbPayBatchData> batches = this.writePlatformService.retrieveBatches(offset, limit);
    ...
    public String batch(@PathParam("batchId") final Long batchId, @Context final UriInfo uriInfo) {
        ...
        this.context.authenticatedUser().validateHasPermissionTo("CREATE_" + SsbConstants.RESOURCE_PAY_IMPORT);
        final SsbPayBatchData batch = this.writePlatformService.retrieveBatch(batchId);
        final ApiRequestJsonSerializationSettings settings = this.apiRequestParameterHelper.process(uriInfo.getQueryParameters());
    ...
    public String rowCommand(@PathParam("batchId") final Long batchId, @PathParam("rowId") final Long rowId,
        ...
        final AppUser user = this.context.authenticatedUser();
        user.validateHasPermissionTo("UPDATE_" + SsbConstants.RESOURCE_PAY_IMPORT);

        final SsbPayRowData row;
