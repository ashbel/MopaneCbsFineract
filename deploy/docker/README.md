...
Build community-app in `mopane-cbs-ui` with `grunt prod`, then copy `dist/community-app/` contents to `/opt/fineract/ui/dist` on the VPS (served by `fineract-legacy-ui`). Details in that repo’s `DEPLOY.md`.
